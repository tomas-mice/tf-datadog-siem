resource "datadog_security_monitoring_rule" "tfer--security_monitoring_rule_ogd-llz-yxt" {
  case {
    condition = "a > 0"
    status    = "info"
  }
  ...
  name               = "[TBOL] IIS HTTP requests from security scanner"
  ...
  query {
    ...
    query           = "source:iis @http.useragent:(*burp* OR *burpcollaborator.net* OR *qualys* OR *nexpose* OR *OpenVAS* OR *Nikto* OR *Meterpreter* OR *IceWeasel* OR *DirB* OR *Comodo* OR *Tripwire* OR *Retina* OR *MBSA* OR *ImmuniWeb* OR *Netsparker* OR *Acunetix* OR *Intruder* OR *nmap* OR *CVE* OR *base64* OR *eval* OR *javascript* OR *alert*)"
  }
